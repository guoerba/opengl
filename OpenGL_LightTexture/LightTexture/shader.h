...
const static char fragmentShader[] = {
    ...
    "void main(){\n"
        ...
        "vec3 ambient = light.ambient*light.light*vec3(texture(material.diffuse,Texcoord));\n "//环境光照

        "vec3 intrudelight = normalize(lightDir - vertexPos);\n "//入射光（方向向量）
        "vec3 norm = normalize(Normal);\n "
        "float diffuseStrength = max(0.0f,dot(norm,intrudelight));\n "//漫反射光照因子
        "vec3 diffuse = diffuseStrength*light.diffuse*light.light*vec3(texture(material.diffuse,Texcoord));\n "//漫反射光照（在观察空间里计算）

        "vec3 reflectlight = normalize(reflect(vertexPos-lightDir,norm));\n "//反射光
//pow(x,y):x的y次方；reflect(L,N):return L-2*dot(L,N)*N; 32是镜面反射强度
        ...
        "vec3 specular = 10*specularStrength*vec3(texture(material.specular,Texcoord))*light.specular*light.light;\n "//镜面反射
