...
void Camera::domovement()
{
    if(state[FORWARD])
    {
        cameraPos += cameraFront.normalized()*cameraSpeed*deltatime;//摄像机现在的位置等于摄像机以前的位置加上摄像机正向量
    }
    if(state[BACKWARD])
    {
        cameraPos -= cameraFront.normalized()*cameraSpeed*deltatime;
    }
    if(state[MOVELEFT])
    {
        cameraPos -= QVector3D::crossProduct(cameraFront,cameraUp).normalized()*cameraSpeed*deltatime;
    }
    if(state[MOVERIGHT])
    {
        cameraPos += QVector3D::crossProduct(cameraFront,cameraUp).normalized()*cameraSpeed*deltatime;
    ...
    if(state[MOVEUP])
    {
        ...
        cameraPos += QVector3D::crossProduct(right,cameraFront).normalized()*cameraSpeed*deltatime;
    }
    if(state[MOVEDOWN])
    {
        QVector3D right = QVector3D::crossProduct(cameraFront,cameraUp).normalized();
