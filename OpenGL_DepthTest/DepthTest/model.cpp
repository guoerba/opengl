...
Mesh Model::processMesh(aiMesh *mesh, const aiScene *scene)
{
    ...
    for(unsigned int i=0;i<mesh->mNumVertices;i++)
    {
        ...
        if(mesh->HasTextureCoords(0))// 网格是否有纹理坐标？
        {
    ...
    qDebug() << "mesh->mMaterialIndex: " << mesh->mMaterialIndex;
...
{
    QVector<MeshTexture> textures;
    qDebug() << tr("%1 textureCount: ").arg(typeName) << material->GetTextureCount(type);
    unsigned int texturesCount = material->GetTextureCount(type);
    if(texturesCount > 0)//该节点网格的纹理数量大于0
    {
        for(unsigned int i=0;i<texturesCount;i++)
        {
            aiString str;
            material->GetTexture(type,i,&str);
            qDebug() << "img path : " << tr(str.C_Str());
            if(str.length != 0)
            {
    ...
    /*else
    {
        ...
        MeshTexture tex;
        QString path = tr(":/texture/texture/box.jpg");
        SetTexture(textureList[texsize - 1], texsize - 1,path);
        tex.texture = textureList[texsize - 1];
        tex.sampler = texsize - 1;
        tex.type = typeName;
        tex.path = path;
